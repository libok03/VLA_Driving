...
def build_dataset(cfg: dict, split: str, max_samples: int = 0) -> MotorTemporalDataset:
    data_cfg = cfg["data"]
    label_cfg = cfg["labels"]
    manifest_key = "train_manifest" if split == "train" else "val_manifest"
    return MotorTemporalDataset(
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train temporal full-LiDAR xycar motor model.")
    parser.add_argument("--config", default="configs/motor_control_temporal.yaml")
    ...
    if len(train_dataset) == 0 or len(val_dataset) == 0:
        raise SystemExit("No temporal motor samples found.")

    model = MotorTemporalGRU(**cfg["model"]).to(device)
